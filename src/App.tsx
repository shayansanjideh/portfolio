    ...
    const fetchTransactions = async () => {
        ...
        if (txs) {
            ...
            for (let tx of txs) {
                ...
                console.log(tx);
                // Outer if loop: if assets are entering the address
                if (tx.to_address === ethAddress) {
                    // Inner if loop: if assets are ERC-20 tokens
                    if (tx.value === "0") {
                        let erc20_args = {
                            address: ethAddress,
                        ...
                        for (let token of erc20_tokens) {
                            ...
                            portfolio_value += token_balance * Math.pow(10, -(token_decimals)) * token_price.usdPrice;
                        }
                        // Inner if loop: if transaction involves native ETH
                } else {
                        ...
                        portfolio_value += eth_balance * eth_price.usdPrice;
                    }
                    // Outer if loop: if assets are leaving the address
                } else {
                    // Inner if loop: if assets are ERC-20 tokens
                    if (tx.value === "0") {
                        let erc20_args = {
